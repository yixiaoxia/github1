...
{   
   ...
   {  super(provider,server_profile);
      if (server_profile.call_log) call_logger=new CallLoggerImpl("//"+provider.getViaAddress()+"."+provider.getPort()+"_calls.log");//SipStack.log_path+
   }
   ...
   {  printLog("inside processRequestToLocalUser(msg)",LogLevel.MEDIUM);
      ...
      if (false)//FIXME if (targets.isEmpty())
