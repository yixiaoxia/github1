...
{   
   ...
   /** When a new request is received for the local server. */ //FIXME 貌似注册地方1
   ...
   /** When a new request message is received for a local user. 404*/
   ...
   /** When a new request message is received for a remote UA. 给客户端发送404错误*/ 
   ...
   /** Gets the request's targets as Vector of String.
     * @return It returns a Vector of String representing the target URLs.  查询目标的url等信息*/
   ...
   {  printLog("inside getTargets(msg)",LogLevel.LOW);
      ...
      if (false)//songchange   if (!location_service.hasUser(user))
   ...
   {  
	   ToHeader th=msg.getToHeader();                        //to:<phone@192.168.0.108>
      if (th==null)  			
      ...
      SipURL dest_uri=th.getNameAddress().getAddress();            //to:<phone@192.168.0.108>
      String user=dest_uri.getUserName()+"@"+dest_uri.getHost();    

      int exp_secs=server_profile.expires;   
      // set the expire value
      ExpiresHeader eh=msg.getExpiresHeader();  //有效时间 3600
      if (eh!=null)
      ...
      if (!msg.hasContactHeader()) //FIXME 修改  if (!msg.hasContactHeader())
      ...
      location_service.sync();  
	   int result1=200;
     Message resp1=MessageFactory.createResponse(msg,result,SipResponses.reasonOf(result),null);  
      return resp1;
   }
