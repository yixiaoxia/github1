...
/**
 * StatefulProxy server. Class StatefulProxy implement a stateful SIP proxy
 * server. It extends class Registrar. A StatefulProxy can work as simply SIP
 * proxy, or it can handle calls for registered users.
 */
public class StatefulProxy extends Proxy implements TransactionClientListener {
	...
	/**
	 * Costructs a new StatefulProxy that acts also as location server for
	 * registered users.
	 */
	/*
	 * public StatefulProxy(SipProvider provider_server, SipProvider
	 * provider_client, ServerProfile server_profile) {
	 * super(provider_server,server_profile);
	 * sip_provider_client=provider_client; sip_provider_server=provider_server;
	 * init(); }
	 */
	...
	public void processRequestToLocalUser(Message msg) {
		...
		if (msg.isAck()) {
			printLog(
					"ACK received out of an active InviteServerTransaction, message forwarded",
					LogLevel.MEDIUM);
			// just send the ack..
		...
		ts = new TransactionServer(sip_provider_server, msg, null);
		// ts.listen();

		if (server_profile.do_proxy_authentication && !msg.isAck()
				&& !msg.isCancel()) { // check message authentication
		...
		if (targets.isEmpty()) { // try to treat the request-URI as a phone URL
			SipURL request_uri = msg.getRequestLine().getAddress();
			SipURL new_target = getPhoneTarget(request_uri);
			if (new_target != null)
				targets.addElement(new_target.toString());
			printLog("No target found, message discarded", LogLevel.HIGH);
			if (!msg.isAck())
				statefulServerResponse(
						ts,
						MessageFactory.createResponse(msg, 404,
								SipResponses.reasonOf(404), null));
			return;
		}

		printLog("message will be forwarded to all user's contacts",
				LogLevel.MEDIUM);
		for (int i = 0; i < targets.size(); i++) {
			SipURL url = new SipURL((String) (targets.elementAt(i)));
			Message request = new Message(msg);
			request.removeRequestLine();
			request.setRequestLine(new RequestLine(msg.getRequestLine()
					.getMethod(), url));
			...
			if (msg.isInvite())
				tc = new InviteTransactionClient(sip_provider_client, request,
						this);
			else
				tc = new TransactionClient(sip_provider_client, request, this);
		...
		for (Iterator i = clients.iterator(); i.hasNext();)
			((TransactionClient) i.next()).request();
	...
	public void processRequestToRemoteUA(Message msg) {
		...
		TransactionServer ts;
		if (msg.isInvite())
			ts = new InviteTransactionServer(sip_provider_server, msg, null);
		else
			ts = new TransactionServer(sip_provider_server, msg, null);
		// ts.listen();

		if (!server_profile.is_open_proxy) { // check whether the caller is a
												// local user
			...
			String caller = (from_username == null) ? from_hostaddr
					: from_username + "@" + from_hostaddr;
			if (!location_service.hasUser(caller)) { // but do not filter
				...
				String callee = (to_username == null) ? to_hostaddr
						: to_username + "@" + to_hostaddr;
				if (!location_service.hasUser(callee)) { // both caller and
															// callee are not
															// registered with
															// the local server
					printLog(
							"both users "
									...
									+ " and "
									+ callee
									+ " are not registered with the local server: proxy denied.",
							LogLevel.HIGH);
					ts.respondWith(MessageFactory.createResponse(msg, 503,
							SipResponses.reasonOf(503), null));
					return;
				}
			}
		}
		...
		TransactionClient tc;
		if (msg.isInvite())
			tc = new InviteTransactionClient(sip_provider_client, msg, this);
		else
			tc = new TransactionClient(sip_provider_client, msg, this);
		state.addClient(ts, tc);
		tc.request();
	}
	...
	protected void statefulServerResponse(TransactionServer ts, Message resp) {
		...
		printLog("Server response: " + resp.getStatusLine().toString(),
				LogLevel.MEDIUM);
		ts.respondWith(resp);
	}

	/** Process provisional response */
	protected void processProvisionalResponse(Transaction transaction,
			Message resp) {
	...
	protected void processFailureResponse(Transaction transaction, Message resp) {
		...
		HashSet clients = state.getClients(ts);
		if (clients.isEmpty()) {
			printLog("only this tr_client remained: send the response",
					LogLevel.LOW);
	...
	protected void processSuccessResponse(Transaction transaction, Message resp) {
		...
		if (resp.hasViaHeader()) {
			...
			printLog("Cancelling " + clients.size() + " pending clients",
					LogLevel.LOW);
			for (Iterator i = clients.iterator(); i.hasNext();) {
				Transaction tc = (Transaction) i.next();
				Message cancel = MessageFactory.createCancelRequest(tc
						.getRequestMessage());
				TransactionClient tr_cancel = new TransactionClient(
						sip_provider_server, cancel, null);
				tr_cancel.request();
			}
			state.removeServer(ts);
		}
	...
	protected void processTimeout(Transaction transaction) {
		...
		if (clients.isEmpty()) {
			...
			updateProxingResponse(resp);
			if (resp.hasViaHeader())
				statefulServerResponse(ts, resp);
			state.removeServer(ts);
		}
	}
	...
	/**
	 * When the TransactionClient is in "Proceeding" state and receives a new
	 * 1xx response
	 */
	...
	/**
	 * When an TransactionClient goes into the "Terminated" state, receiving a
	 * 2xx response
	 */
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < args.length; i++) {
			...
			if (args[i].equals("-h")) {
				...
				System.out
						.println("   -f <config_file> specifies a configuration file");
